...
class FavoritesScreen extends StatefulWidget {
  ...
  const FavoritesScreen({Key? key, required this.showAppBar}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: InkWell(onTap: () {}, child: Text("Favorites")),
          leading: IconButton(
              onPressed: () {
            ...
            children: [
              ...
              (isGetRequest)
                  ? FutureBuilder<FavResponse>(
                      future: appRepository.getFavoritesPost("Pf1PZMTEum3zLBkN2ITu4KdZyHM3WKR0", id, (FavResponse list) {
                      ...
                      builder: (BuildContext context, AsyncSnapshot<FavResponse> snapshot) {
                        switch (snapshot.connectionState) {
                          ...
                          default:
                            ...
                            } else if (snapshot.data!.status == "OK" && snapshot.data!.data != null && snapshot.data!.data.data.length == 0) {
                              isGetRequest = false;
                              print("else iff");
                              return Container(
                                ...
                                child: Center(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      "No Favorites Yet.",
                                      style: GoogleFonts.montserrat(color: Color(0xff999999), fontWeight: FontWeight.w500, fontSize: 18),
                                    )
                                  ],
                                )),
                              );
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      GestureDetector(
                                        onTap: () {
                                          EasyLoading.show();
                                          AuthProvider().addfavourite(user[index].id, 1).then((res) {
                                            provider.updateStoriesFavStatus(
                                              index,
                                            );

                                            setState(() {
                                              user[index].isFavourite = user[index].isFavourite.contains("1") ? "0" : "1";
                                            });
                                        ...
                                        child: Icon(
                                          user[index].isFavourite == "1" ? Icons.favorite : Icons.favorite_border_outlined,
                                          color: user[index].isFavourite == "1" ? Colors.red : Colors.grey,
                                          size: 20,
                                        ),
                                      )
                                    ],
